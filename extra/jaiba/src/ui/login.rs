...
pub fn draw_login(frame: &mut Frame, app: &mut App) {
    ...
    let logo_height = if app.slim_mode {
        0
    } else {
        20
    };
    ...
    let _fingerprint_hint = if app.fingerprint_verifying {
        Some(Line::from(Span::styled(
            "Scanning fingerprint...",
            Style::new().fg(app.theme.warning),
        )))
    } else if app.config.fingerprint_enabled && app.fprint_available && !app.creating_database && !missing {
        Some(Line::from(Span::styled(
            "[^f] fingerprint unlock",
    ...
    if let Some(error) = &app.login_error {
    ...
    } else if app.fingerprint_verifying {
        // Show fingerprint scanning status
        let hint_area = Rect {
            x: input_area.x,
            y: input_area.y + input_area.height,
            width: input_area.width,
        ...
        let hint = Paragraph::new("Scanning fingerprint...")
            .alignment(Alignment::Center)
        ...
        // Normal login screen - show fingerprint hint if enabled
        if app.config.fingerprint_enabled && app.fprint_available {
            let hint_area = Rect {
                x: input_area.x,
                y: input_area.y + input_area.height,
                width: input_area.width,
            ...
            let hint = Paragraph::new("[^f] fingerprint unlock  [esc] quit")
                .alignment(Alignment::Center)
                .style(Style::new().fg(app.theme.warning));
