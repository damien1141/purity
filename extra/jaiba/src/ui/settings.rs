...
use crate::input::settings::{
    AUTO_LOCK_ROW, CLIPBOARD_TIMEOUT_ROW, DATABASE_ROW, KEYFILE_ROW,
};
...
pub fn draw_settings(frame: &mut Frame, app: &mut App) {
    if app.entering_fprint_password {
        draw_fprint_password(frame, app);
    } else if app.exporting_database {
...
fn draw_main_settings(frame: &mut Frame, app: &mut App) {
    ...
    let sort_mode_value = app.config.sort_mode.as_str();
    let group_display_value = app.config.group_display.as_str();
    let logo_style_value = app.config.logo_style.as_str();

    let fingerprint_value = if app.config.fingerprint_enabled {
        if app.fprint_available {
            if app.fprint_enrolled {
                if crate::fprint::stored_password_exists(&app.config) {
                    Line::from(Span::styled("enabled (enrolled)", normal))
                } else {
                    Line::from(Span::styled(
                        "enabled (password missing — toggle off/on to repair)",
                        warning,
                    ))
                }
            } else {
                Line::from(Span::styled("enabled (no prints)", warning))
            }
        } else {
            Line::from(Span::styled("enabled (fprintd missing)", warning))
        }
    } else {
        Line::from(Span::styled("disabled", placeholder))
    };
    ...
    let items = vec![
        ...
        field(
            "Sort mode",
            Line::from(Span::styled(sort_mode_value, normal)),
        ),
        field(
            "Group display",
            Line::from(Span::styled(group_display_value, normal)),
        ),
        field(
            "Logo style",
            Line::from(Span::styled(logo_style_value, normal)),
        ),
        field("Fingerprint", fingerprint_value),
    ];
...
fn draw_fprint_password(frame: &mut Frame, app: &mut App) {
    let full_area = frame.area();
    ...
    app.max_len = input_area.width.saturating_sub(2) as usize;

    let input = Paragraph::new("•".repeat(app.fprint_password_buffer.chars().count()))
        .alignment(Alignment::Center)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(app.theme.border))
                .title(" Master password for fingerprint "),
        );
    ...
    let hint_text = app
        .status
        .clone()
        .unwrap_or_else(|| "this stores the password used for fingerprint unlock".to_string());
    ...
    let typed_len = app.fprint_password_buffer.chars().count() as u16;
    let inner_width = input_area.width.saturating_sub(1);
...
fn draw_theme_picker(frame: &mut Frame, app: &mut App) {
    ...
    let help_width = full_area.width.saturating_sub(2);
    let help_lines = wrap_help_items(theme_picker_help_items(app.slim_mode), help_width);
    let help_height = help_lines.len() as u16;
