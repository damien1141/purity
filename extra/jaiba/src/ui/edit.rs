...
pub fn draw_edit(frame: &mut Frame, app: &mut App) {
    ...
    for (key, value) in entry.custom_fields.clone() {
        ...
        if value.is_empty() {
            ...
            for wrapped_line in wrap_notes(&value, notes_width) {
                lines.push(Line::from(Span::styled(wrapped_line, normal)));
            }
        }
