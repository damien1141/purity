...
use crate::config::{save_config, GroupDisplay, SortMode};
...
pub const SORT_MODE_ROW: usize = 8;
pub const GROUP_DISPLAY_ROW: usize = 9;
pub const LOGO_STYLE_ROW: usize = 10;
pub const FINGERPRINT_ROW: usize = 11;

pub fn handle_settings_input(app: &mut App, key: KeyCode) {
    if app.entering_fprint_password {
        handle_fprint_password_input(app, key);
        return;
    }
...
fn activate_selected(app: &mut App) {
    ...
    match selected {
        ...
        SORT_MODE_ROW => cycle_sort_mode(app),
        GROUP_DISPLAY_ROW => cycle_group_display(app),
        LOGO_STYLE_ROW => cycle_logo_style(app),
        FINGERPRINT_ROW => toggle_fingerprint(app),
        _ => {}
    }
}

fn toggle_fingerprint(app: &mut App) {
    app.status = None;
    app.refresh_fprint_status();

    if app.config.fingerprint_enabled {
        app.config.fingerprint_enabled = false;
        app.config.fingerprint_device = None;

        let _ = fprint::delete_stored_password(&app.config);
        app.refresh_fprint_status();

        app.status = Some(match save_config(&app.config) {
            Ok(()) => "fingerprint disabled".to_string(),
            Err(err) => format!("couldn't save config: {err}"),
        });
    } else {
        if !app.fprint_available {
            app.status = Some("fprintd is not installed".to_string());
            return;
        }

        if !app.fprint_enrolled {
            app.status = Some(
                "No fingerprints enrolled — run fprintd-enroll in another terminal, then toggle again"
                    .to_string(),
            );
            return;
        }
        ...
        if app.config.default_database.is_none() {
            app.status = Some("set a default database first".to_string());
            return;
        }

        app.entering_fprint_password = true;
        app.fprint_password_buffer.clear();
        app.status = Some("Enter current master password to enable fingerprint unlock".to_string());
    }
}

fn handle_fprint_password_input(app: &mut App, key: KeyCode) {
    match key {
        KeyCode::Char(c) => {
            app.status = None;
            let limit = app.max_len.max(1);
            if app.fprint_password_buffer.len() < limit {
                app.fprint_password_buffer.push(c);
            }
        }
        KeyCode::Backspace => {
            app.status = None;
            app.fprint_password_buffer.pop();
        }
        KeyCode::Enter => confirm_fprint_password(app),
        KeyCode::Esc => cancel_fprint_password(app),
        _ => {}
    }
}

fn cancel_fprint_password(app: &mut App) {
    app.entering_fprint_password = false;
    app.fprint_password_buffer.clear();
    app.status = None;
}

fn confirm_fprint_password(app: &mut App) {
    let Some(path) = app.config.default_database.clone() else {
        app.status = Some("no default database configured".to_string());
        cancel_fprint_password(app);
        return;
    };

    let password = std::mem::take(&mut app.fprint_password_buffer);
    ...
    match unlock_database(&path, &password, app.config.keyfile.as_deref()) {
        Ok(_) => {
            if let Err(err) = fprint::store_password(&app.config, &password) {
                app.status = Some(format!("couldn't store fingerprint password: {err}"));
                return;
            }

            app.config.fingerprint_enabled = true;
            app.refresh_fprint_status();

            match save_config(&app.config) {
                Ok(()) => {
                    app.entering_fprint_password = false;
                    app.status = Some("fingerprint enabled".to_string());
                }
                Err(err) => {
                    app.config.fingerprint_enabled = false;
                    let _ = fprint::delete_stored_password(&app.config);
                    ...
                    app.status = Some(format!(
                        "fingerprint password stored, but couldn't save config: {err}"
                    ));
                }
            }
        }
        Err(err) => {
            app.status = Some(format!("couldn't verify master password: {err}"));
            app.fprint_password_buffer.clear();
        }
    }
}

fn cycle_sort_mode(app: &mut App) {
    app.status = None;
    let modes = [SortMode::Smart, SortMode::Name, SortMode::Recent, SortMode::Reuse, SortMode::LastCopied];
    let current = &app.config.sort_mode;
    let next = modes
        .iter()
        .position(|m| m == current)
        .and_then(|i| modes.get((i + 1) % modes.len()))
        .cloned()
        .unwrap_or(SortMode::Smart);

    app.config.sort_mode = next;
    app.refresh_filter();

    app.status = Some(match save_config(&app.config) {
        Ok(()) => format!("sort mode: {}", app.config.sort_mode.as_str()),
        Err(err) => format!("couldn't save config: {err}"),
    });
}

fn cycle_group_display(app: &mut App) {
    app.status = None;
    let modes = [
        GroupDisplay::Shown,
        GroupDisplay::Hidden,
    ...
    let current = &app.config.group_display;
    let next = modes
        .iter()
        .position(|m| m == current)
        .and_then(|i| modes.get((i + 1) % modes.len()))
        .cloned()
        .unwrap_or(GroupDisplay::Shown);

    app.config.group_display = next;

    if next == GroupDisplay::Hidden {
    ...
    app.status = Some(match save_config(&app.config) {
        Ok(()) => format!("group display: {}", app.config.group_display.as_str()),
        Err(err) => format!("couldn't save config: {err}"),
    });
}

fn cycle_logo_style(app: &mut App) {
    app.status = Some("logo style: crab (only option)".to_string());
}
...
fn commit_password_change(app: &mut App) {
    ...
    match save_database(&path, &new_key, db, &mut app.entries) {
        Ok(()) => {
            app.db_key = Some(new_key);

            if app.config.fingerprint_enabled {
                if let Err(err) = fprint::store_password(&app.config, &app.new_password_buffer) {
                    app.status = Some(format!(
                        "master password changed, but couldn't update fingerprint password: {err}"
                    ));
                    app.changing_password = false;
                    app.password_change_step = PasswordChangeStep::CurrentPassword;
                    app.current_password_buffer.clear();
            ...
            app.status = Some("master password changed".to_string());
        }
        Err(err) => {
            app.status = Some(format!("couldn't change password: {err:#}"));
        }
