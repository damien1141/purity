...
pub fn import_kdbx(path: &Path, password: &str) -> anyhow::Result<Vec<Entry>> {
    let (_, _, entries, _) = unlock_database(path, password, None)?;
    Ok(entries)
}
...
mod tests {
    ...
    #[test]
    fn imports_empty_bitwarden_export_as_zero_entries() {
        let dir = std::env::temp_dir();
        let path = dir.join("jaiba-bitwarden-empty.json");

        fs::write(&path, r#"{"items": []}"#).expect("write test file");

        let entries = import_json(&path).expect("empty Bitwarden export should import");
