...
- **Smart sorting**: entries are ranked by a composite score of recency, security warnings, and active usage signals. Sort mode is configurable in Settings.
- **Flexible group display**: choose how folders/groups appear in the index — shown above entries, hidden entirely, placed at the bottom, or sorted alongside entries. Configurable in Settings.
- **Fingerprint unlock**: unlock your vault with a fingerprint via `fprintd` (press `^f` on the login screen). Configurable in Settings.
...
```toml
default_database = "~/Documents/passwords.kdbx"
keyfile = "~/Documents/passwords.keyx" # optional; omit for password-only vaults
auto_lock = 300           # seconds of inactivity before locking
clipboard_timeout = 15    # seconds before a copied value is cleared
theme = "catppuccin-mocha"
sort_mode = "smart"       # smart, name, recent, reuse, lastcopied
logo_style = "crab"   # crab
group_display = "shown"   # shown, hidden, bottom, mixed
fingerprint_enabled = false # enable fingerprint unlock via fprintd
fingerprint_device = ""     # optional; leave empty for default device
```

- **Group display**: control how folders/groups appear in the entry index. `shown` places groups above entries (default), `hidden` hides groups entirely, `bottom` puts groups after entries, and `mixed` sorts groups alongside entries alphabetically.
