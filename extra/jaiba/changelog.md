...
- Smart auto-sorting for entries: ranks by recency, security warnings (password/user reuse), and active usage signals (TOTP, URL). Configurable via Settings or `sort_mode` in config.
- Fingerprint unlock via fprintd: press `^f` on login screen to unlock with fingerprint. Enrollment and toggle via Settings. Requires `fprintd` installed and enrolled fingerprints.
...
- Dropping support for packages, now the official install is coming from cargo.
- Trimming the readme file so it can actually be digested.
