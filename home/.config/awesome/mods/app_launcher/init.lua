...
local function quicksettings()
    ...
    local function update_night_visual()
        night_icon.markup = helpers.colorize_text("", night_state and beautiful.accent or beautiful.fg_color .. "4D")
...
local time_hour = wibox.widget{
    font = beautiful.font_var and (beautiful.font_var .. "Bold 13") or "Sans Bold 13",
    format = "%H:",
...
local function create_clipboard_widget(self, entry)
    local accent = beautiful.accent or beautiful.fg_color
    local icon_glyph = entry.is_image and "" or ""
    local display_text = tostring(entry.text or ""):gsub("[\r\n]+", " ")
                ...
                {
                    {
                        widget = wibox.widget.textbox,
                        font = beautiful.icon_var .. "12",
                        markup = helpers.colorize_text(icon_glyph, accent),
                        align = "center",
                        valign = "center",
                    },
                    widget = wibox.container.background,
                    ...
                    bg = (#accent == 7) and (accent .. "1A") or accent,
                },
                {
                    widget = wibox.widget.textbox,
                    id = "name",
...
notification_mode.max_items = 8

local function safe_destroy(n)
    if n and n.destroy then pcall(function() n:destroy() end) end
end
        ...
        {
            {
                ...
                {
                    widget = wibox.widget.textbox,
                    markup = helpers.colorize_text("all caught up", beautiful.fg_color .. "99"),
...
function notification_mode.create_notification_widget(self, n)
    ...
    local message_text = (n.message or ""):gsub("%s+", " ")
    message_text = message_text:gsub("^%s+", ""):gsub("%s+$", "")
    if #message_text > 0 then
        if #message_text > 160 then message_text = message_text:sub(1, 157) .. "…" end
        table.insert(content_col, {
            widget = wibox.widget.textbox,
            markup = helpers.colorize_text(message_text, beautiful.fg_color .. "CC"),
    ...
    if n.actions and #n.actions > 0 then
        ...
        local action_hover = (#accent == 7) and (accent .. "33") or beautiful.bg_focus
        for _, action in ipairs(n.actions) do
            local btn = wibox.widget {
                {
                    {
                        widget = wibox.widget.textbox,
                        markup = helpers.colorize_text((action.name or "?"):gsub("^_", ""), beautiful.fg_color),
                        font = beautiful.font_var .. "9",
                        align = "center",
                        valign = "center",
                    },
                    widget = wibox.container.margin,
                    left = dpi(10), right = dpi(10), top = dpi(3), bottom = dpi(3),
                },
                widget = wibox.container.background,
                bg = beautiful.bg_frost_2,
            ...
            btn:connect_signal("button::press", function()
                pcall(function() action:invoke(n) end)
                notification_mode.remove_notification(self, n)
                safe_destroy(n)
                self:hide()
            end)
            ...
            btn:connect_signal("mouse::leave", function()
                btn.bg = beautiful.bg_frost_2
                if capi.mouse.current_wibox then capi.mouse.current_wibox.cursor = "left_ptr" end
            end)
            table.insert(action_row, btn)
        end
        table.insert(content_col, action_row)
    end
    ...
    local item = wibox.widget {
        ...
        shape = helpers.rrect(beautiful.rounded or 6),
        {
            widget = wibox.container.margin,
            margins = dpi(10),
            {
    ...
    item:connect_signal("mouse::enter", function()
        local idx = notification_mode.index_of(self, item)
        if idx then
            self._private.notification_selected_row = idx
            notification_mode.update_highlight(self, idx)
        end
        if capi.mouse.current_wibox then capi.mouse.current_wibox.cursor = "hand2" end
    end)
    item:connect_signal("mouse::leave", function()
    ...
    item:connect_signal("button::press", function(_, _, button)
        local idx = notification_mode.index_of(self, item)
        if not idx then return end
        self._private.notification_selected_row = idx
        if button == 1 then
            notification_mode.select_active(self)
            self:hide()
        elseif button == 3 then
            notification_mode.dismiss_notification(self, idx)
        end
    end)
    return item
end
...
function notification_mode.add_notification(self, n)
    ...
    table.insert(self._private.notification_items, { widget = item, notification = n })
    while #self._private.notification_items > notification_mode.max_items do
        local dropped = table.remove(self._private.notification_items, 1)
        safe_destroy(dropped.notification)
    end
    notification_mode.rebuild(self)
end

function notification_mode.index_of(self, widget)
    for i, entry in ipairs(self._private.notification_items or {}) do
        if entry.widget == widget then return i end
    end
    return nil
end

function notification_mode.rebuild(self)
    local container = self._private.notification_container
    if not container then return end
    container:reset()
    local items = self._private.notification_items or {}
    if #items == 0 then
        if self._private.notification_empty then container:add(self._private.notification_empty) end
    else
        for _, entry in ipairs(items) do
            container:add(entry.widget)
        end
    end
    local row = self._private.notification_selected_row or 1
    if #items == 0 then row = 1 elseif row > #items then row = #items elseif row < 1 then row = 1 end
    self._private.notification_selected_row = row
    notification_mode.update_highlight(self, row)
end

function notification_mode.remove_notification(self, n)
    local items = self._private.notification_items
    if not items then return end
    for i, entry in ipairs(items) do
        if entry.notification == n then
...
function notification_mode.dismiss_notification(self, row)
    local items = self._private.notification_items
    if not items or row < 1 or row > #items then return end
    local entry = table.remove(items, row)
    notification_mode.rebuild(self)
    safe_destroy(entry.notification)
end

function notification_mode.dismiss_active(self)
    notification_mode.dismiss_notification(self, self._private.notification_selected_row or 1)
end
...
function notification_mode.update_highlight(self, selected_row)
    selected_row = selected_row or self._private.notification_selected_row or 1
    local accent = beautiful.accent or beautiful.fg_focus or "#ffffff"
    local normal_bg = beautiful.bg_frost_3
    local active_bg = (#accent == 7) and (accent .. "14") or normal_bg
    for i, entry in ipairs(self._private.notification_items or {}) do
        if entry.widget then
            entry.widget.bg = (i == selected_row) and active_bg or normal_bg
        end
    end
end
...
function notification_mode.select_active(self)
    ...
    local row = self._private.notification_selected_row or 1
    if not items or row < 1 or row > #items then return end
    local entry = items[row]
    local n = entry.notification
    if n and n.actions and #n.actions > 0 then
        pcall(function() n.actions[1]:invoke(n) end)
    end
    notification_mode.remove_notification(self, n)
    safe_destroy(n)
end

local EMOJI_PAGE_SIZE = 75
local EMOJI_COLS = 5

local function emoji_fuzzy_filter(query)
    query = (query or ""):lower()
    if query == "" or query == ":" then return emoji.emojis end
    local scored = {}
    for i, em in ipairs(emoji.emojis) do
        local name = em.name:lower()
        ...
        elseif name:sub(1, #query) == query then
            ...
            local qi, s, prev_hit, word_start = 1, 2, false, true
            for p = 1, #name do
                if qi > #query then break end
                local c = name:sub(p, p)
                if c == "_" or c == " " then
                    word_start, prev_hit = true, false
                elseif c == query:sub(qi, qi) then
                    if word_start then s = s + 1
                    elseif prev_hit then s = s + 2
                    else s = s + 4 end
                    qi, word_start, prev_hit = qi + 1, false, true
                else
                    word_start, prev_hit = false, false
                end
            end
            if qi > #query then score = s + math.floor((#name - #query) / 4) end
        end
        if score then table.insert(scored, { em = em, score = score, idx = i }) end
    end
    ...
    local out = {}
    for i, v in ipairs(scored) do out[i] = v.em end
    return out
end

local function emoji_set_selected(self, idx)
    local filtered = self._private.emoji_filtered or {}
    if #filtered == 0 then return end
    idx = math.max(1, math.min(idx, #filtered))
    local prev = self._private.emoji_selected
    if idx == prev then return end
    self._private.emoji_selected = idx
    local cells = self._private.emoji_cells or {}
    if prev and cells[prev] then cells[prev].bg = self.app_normal_color or beautiful.bg_normal end
    if cells[idx] then cells[idx].bg = self.app_selected_color or beautiful.bg_focus end
end

local function render_emoji_grid(self)
    local grid = self._private.emoji_grid
    grid:reset()
    self._private.emoji_cells = {}
    local filtered = self._private.emoji_filtered or {}
    if #filtered == 0 then self._private.emoji_selected = nil; return end
    local selected = math.min(self._private.emoji_selected or 1, #filtered)
    self._private.emoji_selected = selected
    local offset = math.floor((selected - 1) / EMOJI_PAGE_SIZE) * EMOJI_PAGE_SIZE
    self._private.emoji_offset = offset
    local limit = math.min(#filtered - offset, EMOJI_PAGE_SIZE)
    local selected_color = self.app_selected_color or beautiful.bg_focus
    local normal_color = self.app_normal_color or beautiful.bg_normal
    for i = 1, limit do
        local idx = offset + i
        ...
        local bg = wibox.widget {
            ...
            shape = self.app_shape or gears.shape.rounded_rect,
            bg = (idx == selected) and selected_color or normal_color,
            { widget = wibox.widget.textbox, text = em.unicode, align = "center", valign = "center", font = "Noto Color Emoji 18", ellipsize = "none" }
        }
        local cell = wibox.widget { widget = wibox.container.margin, margins = dpi(2), bg }
        awful.tooltip { objects = { cell }, text = em.name, mode = "outside", align = "top", preferred_positions = { "top", "bottom", "right", "left"} }
        cell:connect_signal("button::press", function()
        ...
        cell:connect_signal("mouse::enter", function()
            emoji_set_selected(self, idx)
            if capi.mouse.current_wibox then capi.mouse.current_wibox.cursor = "hand2" end
        end)
        cell:connect_signal("mouse::leave", function()
            if capi.mouse.current_wibox then capi.mouse.current_wibox.cursor = "left_ptr" end
        end)
        self._private.emoji_cells[idx] = bg
        grid:add(cell)
    end
end

local function emoji_move(self, delta)
    local filtered = self._private.emoji_filtered or {}
    if #filtered == 0 then return end
    local sel = self._private.emoji_selected or 1
    local target = math.max(1, math.min(sel + delta, #filtered))
    if target == sel then return end
    local old_page = math.floor((sel - 1) / EMOJI_PAGE_SIZE)
    local new_page = math.floor((target - 1) / EMOJI_PAGE_SIZE)
    if new_page ~= old_page then
        self._private.emoji_selected = target
        render_emoji_grid(self)
    else
        emoji_set_selected(self, target)
    end
end
...
local function new(args)
    ...
    ret._private.prompt = prompt {
        ...
        changed_callback = function(text)
            ...
            if is_math then
                ret._private.grid.visible = false; ret._private.emoji_grid.visible = false; ret._private.clipboard_grid.visible = false;                 ret._private.math_widget.visible = true; ret._private.notification_widget.visible = false
                ...
                evaluate_math(text, function(result)
                    ...
                    if result_widget then
                        ...
                        result_widget.markup = string.format("<span foreground='%s'>= %s</span>", color_res, gstring.xml_escape(result))
                    end
                end)
            elseif is_clipboard then
                ret._private.grid.visible = false; ret._private.emoji_grid.visible = false;                 ret._private.clipboard_grid.visible = true; ret._private.notification_widget.visible = false
            ...
            elseif is_power_profile then
                ...
                ret._private.refresh_rate_widget.visible = false; ret._private.notification_widget.visible = false
                ret._private.power_profiles_widget.visible = true
                ret._private.active_grid = nil
                power_profiles_mode.handle_search(ret, text)
            elseif is_refresh_rate then
                ret._private.grid.visible = false; ret._private.emoji_grid.visible = false; ret._private.clipboard_grid.visible = false;                 ret._private.math_widget.visible = false
                ret._private.power_profiles_widget.visible = false; ret._private.notification_widget.visible = false
            ...
            elseif is_emoji then
                ...
                ret._private.power_profiles_widget.visible = false; ret._private.refresh_rate_widget.visible = false; ret._private.math_widget.visible = false; ret._private.notification_widget.visible = false
                ret._private.active_grid = ret._private.emoji_grid
                ret._private.emoji_filtered = emoji_fuzzy_filter(text:sub(2))
                ret._private.emoji_selected = 1
                ret._private.emoji_offset = 0
        ...
        keypressed_callback = function(mod, key, cmd)
            ...
            if key == "Return" or key == "KP_Enter" then
                ...
                elseif ret._private.emoji_mode then
                    local selected_emoji = ret._private.emoji_filtered and ret._private.emoji_filtered[ret._private.emoji_selected or 1]
                    if selected_emoji then
                        local copy_cmd = string.format("printf '%%s' '%s' | xclip -selection clipboard 2>/dev/null || printf '%%s' '%s' | xsel --clipboard --input 2>/dev/null || printf '%%s' '%s' | wl-copy 2>/dev/null || copyq add '%s' 2>/dev/null", selected_emoji.unicode, selected_emoji.unicode, selected_emoji.unicode, selected_emoji.unicode)
            ...
            elseif ret._private.notification_mode then
                ...
                if key == "Delete" then notification_mode.dismiss_active(ret); return true end
                if mod.Shift and key == "BackSpace" then notification_mode.clear_all(ret); return true end
            elseif ret._private.emoji_mode then
                if key == "Up" then emoji_move(ret, -EMOJI_COLS)
                elseif key == "Down" then emoji_move(ret, EMOJI_COLS)
                elseif key == "Left" then emoji_move(ret, -1)
                elseif key == "Right" then emoji_move(ret, 1)
                elseif key == "Prior" then emoji_move(ret, -EMOJI_PAGE_SIZE)
                elseif key == "Next" then emoji_move(ret, EMOJI_PAGE_SIZE)
                end
    ...
    ret._private.emoji_grid = wibox.widget {
        layout = wibox.layout.grid, orientation = "horizontal", homogeneous = true, expand = true, spacing = dpi(8),
        forced_num_rows = 15, forced_num_cols = 5, forced_width = grid_width, forced_height = grid_height, visible = false,
        buttons = {
            awful.button({}, 4, function() emoji_move(ret, -EMOJI_COLS) end),
            awful.button({}, 5, function() emoji_move(ret, EMOJI_COLS) end)
        }
    }
    ...
    ret._private.clipboard_grid = wibox.widget {
        ...
        homogeneous = true, expand = true, spacing = dpi(8), forced_num_cols = 1, visible = false,
    ...
    ret._private.math_widget = wibox.widget {
        ...
        forced_height = ret.app_height,
        shape = ret.app_shape or gears.shape.rounded_rect,
        bg = ret.app_normal_color or beautiful.bg_normal,
        visible = false,
        {
            widget = wibox.container.margin,
            margins = ret.app_content_padding or dpi(10),
            {
                widget = wibox.container.place,
                valign = "center",
                {
                    ...
                    spacing = dpi(12),
                    {
                        widget = wibox.widget.textbox,
                        font = beautiful.icon_var .. "12",
                        markup = helpers.colorize_text("", beautiful.accent or beautiful.fg_color),
                        valign = "center",
                    },
                    {
                        widget = wibox.widget.textbox,
                        id = "math_result",
                        font = ret.app_name_font or beautiful.font,
                        valign = "center",
    ...
    ret._private.widget = awful.popup {
        ...
        shape_border_width = dpi(1), shape_border_color = beautiful.fg_color .. "14",
        widget = {
            {
                {
                    {
                ...
                widget = wibox.container.background, bg = beautiful.bg_frost_2, forced_height = dpi(70)
            },
            {
                layout = wibox.layout.fixed.vertical,
                {
