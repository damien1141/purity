...
local function timed(args)
	...
	local function set(value)
		...
		if obj.override_simulate or (obj._b ~= 0 and obj._b / math.abs(obj._b) ~= obj._m / math.abs(obj._m)) then
			...
			--check for div by 0 resulting in NaN or +-inf; either poisons pos every frame
			if obj._coef ~= obj._coef or obj._coef == math.huge or obj._coef == -math.huge then obj._coef = 1 end
		end
