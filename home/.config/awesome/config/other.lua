...
naughty.connect_signal("request::display_error", function(message, startup)
    naughty.notification {
        app_name = "AwesomeWM",
        urgency = "critical",
        title   = "出错了！" .. (startup and " during startup" or "!"),
        message = message
    }
end)
...
client.connect_signal("request::manage", function(c)
    ...
    if not c.icon then
        if beautiful.images and beautiful.images.awesome then
...
bling.widget.task_preview.enable {
    ...
    width = 200,
    placement_fn = function(c)
        -- Align with right-side bar (width 64 + gap)
        awful.placement.top_right(c, {
            margins = {
                right = dpi(64) + beautiful.useless_gap * 2,
                top = dpi(215)
            }
        })
    end
