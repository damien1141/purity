...
function helpers.colorize_text(txt, fg)
    -- If fg (foreground) color is empty, set it to white (#ffffff)
    fg = fg or "#ffffff"
...
-- Function to colorize text with a given hex code
function colorizeText(text, hexCode)
    return '<span foreground="' .. hexCode .. '">' .. text .. '</span>'
end
    ...
    elseif action == "web_search" then
        awful.prompt.run {
            ...
            exe_callback = function(input)
                ...
                naughty.notify { title = "正在搜索", text = input, icon = nil, urgency = "low", timeout = 2 }
            end
        }
    end
end
