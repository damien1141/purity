...
local search_text = wibox.widget({
    markup = helpers.colorize_text("搜索", beautiful.fg_color .. "99"),
    align = "center",
    valign = "center",
...
local function activate_prompt(action)
    ...
    local prompt
    if action == "run" then
        prompt = generate_prompt_icon("运行:", beautiful.fg_color)
    elseif action == "web_search" then
        prompt = generate_prompt_icon("网页:", beautiful.fg_color)
    end
    helpers.prompt(action, search_text, prompt, function()
        search_icon.visible = true
        search_text.markup = helpers.colorize_text("搜索", beautiful.fg_color .. "99")
    end)
end
