...
local weather_widget_container = wibox.widget{
    {
        weather_icon,
        weather_temp,
        spacing = dpi(4),
    ...
    margins = {left = dpi(0), right = dpi(8)},
...
local fg_color_transparent = fg_color .. "99"

time_hour.markup = helpers.colorize_text(time_hour.text, fg_color)
time_min.markup = helpers.colorize_text(time_min.text, fg_color)
time_mon.markup = helpers.colorize_text(time_mon.text, fg_color_transparent)
time_year.markup = helpers.colorize_text(time_year.text, fg_color_transparent)
time_day.markup = helpers.colorize_text(time_day.text, fg_color_transparent)
time_date.markup = helpers.colorize_text(time_date.text, fg_color_transparent)
...
time_mon:connect_signal("widget::redraw_needed", function()
    time_mon.markup = helpers.colorize_text(time_mon.text, fg_color_transparent)
end)
time_year:connect_signal("widget::redraw_needed", function()
    time_year.markup = helpers.colorize_text(time_year.text, fg_color_transparent)
end)
...
-- Weather widget (placed next to time)
...
local widget_box = wibox.widget{
    ...
    bg = (beautiful.bg_3 or "#333333") .. "cc"
}
...
widget_box:connect_signal("mouse::leave", function ()
    widget_box.bg = (beautiful.bg_3 or "#333333") .. "cc"
end)
