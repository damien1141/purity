...
local function history_check_load(id, max)
    if id and id ~= "" and not data.history[id] then
        ...
        if max then data.history[id].max = max end
        local f = io.open(id, "r")
        if not f then return end
        for line in f:lines() do
            if gtable.hasitem(data.history[id].table, line) == nil then
                table.insert(data.history[id].table, line)
                if #data.history[id].table >= data.history[id].max then break end
            end
        end
        f:close()
    end
...
local function is_word_char(c)
    if c and string.find(c, "[{[(,.:;_-+=@/ ]") then return false else return true end
end

local function cword_start(s, pos)
    local i = pos
    if i > 1 then i = i - 1 end
    while i >= 1 and not is_word_char(s:sub(i, i)) do i = i - 1 end
    while i >= 1 and is_word_char(s:sub(i, i)) do i = i - 1 end
...
local function cword_end(s, pos)
    local i = pos
    while i <= #s and not is_word_char(s:sub(i, i)) do i = i + 1 end
    while i <= #s and is_word_char(s:sub(i, i)) do i = i + 1 end
    return i
end

local function history_save(id)
    if id and data.history[id] then
        gfs.make_parent_directories(id)
        local f = io.open(id, "w")
        if not f then gdebug.print_warning("Failed to write the history to "..id); return end
        for i = 1, math.min(#data.history[id].table, data.history[id].max) do f:write(data.history[id].table[i] .. "\n") end
        f:close()
    end
end

local function history_items(id)
    if id and data.history[id] then return #data.history[id].table else return -1 end
end

local function history_add(id, command)
    if id and data.history[id] and command ~= "" then
        local index = gtable.hasitem(data.history[id].table, command)
        if index == nil then
            table.insert(data.history[id].table, command)
            if #data.history[id].table > data.history[id].max then table.remove(data.history[id].table, 1) end
            history_save(id)
        else
...
local function prompt_text_with_cursor(args)
    ...
    if args.emoji_mode then
        _prompt = "Select an emoji: "
        char = "▮"
        spacer = " "
        text_start = ""
        text_end = ""
        local text_color = gcolor.ensure_pango_color(args.text_color)
        return _prompt .. " <span background=\"#333333\" foreground=\"" .. text_color .. "\">" .. char .. "</span> " .. spacer
    end
    ...
    elseif #text < args.cursor_pos then
        ...
        local offset = 0
        if have_multibyte_char_at(text, args.cursor_pos) then offset = 1 end
    ...
    if args.highlighter then text_start, text_end = args.highlighter(text_start, text_end) end

    ret = _prompt .. text_start .. " <span background=\"" .. cursor_color .. "\" foreground=\"" .. text_color .. "\" underline=\"" .. underline .. "\">" .. char .. "</span>" .. text_end .. spacer
    return ret
end
...
local function update(self)
    ...
    self.textbox:set_markup(prompt_text_with_cursor{
        ...
        prompt = self.prompt, highlighter = self.highlighter, emoji_mode = self._private and self._private.emoji_mode
    })
end
...
function prompt:start()
    ...
    self._private.grabber = keygrabber.run(function(modifiers, key, event)
        ...
        if event ~= "press" then
            if self.keyreleased_callback then self.keyreleased_callback(mod, key, self.command) end
            return
        end
        ...
        if self.keypressed_callback then
            ...
            if new_command or new_prompt then
                if new_command then self.command = new_command end
                if new_prompt then self.prompt = new_prompt end
                update(self)
            end
            if user_catched then
                if self.changed_callback then self.changed_callback(self.command) end
                return
            end
        end
        ...
        if self.hooks[key] then
            for _, m in ipairs(modifiers) do
                if not gtable.hasitem(akey.ignore_modifiers, m) then table.insert(filtered_modifiers, m) end
            end
            for _,v in ipairs(self.hooks[key]) do
                if #filtered_modifiers == #v[1] then
                    local match = true
                    for _,v2 in ipairs(v[1]) do match = match and mod[v2] end
                    if match then
                        local cb
                        ...
                        if ret and type(ret) ~= "boolean" then
                            ...
                            if not quit then self._private_cur_pos = ret:wlen() + 1; update(self) end
                        elseif quit then
                            cb = function() end
                        end
                        if cb then exec(self, cb, original_command) end
                        return
                    end
                end
            end
        ...
        if mod.Control then
            ...
            if key == "v" then
                ...
                if selection then
                    ...
                    if n then selection = selection:sub(1, n - 1) end
            ...
            elseif key == "a" then self._private_cur_pos = 1
            elseif key == "b" then
                if self._private_cur_pos > 1 then
                    self._private_cur_pos = self._private_cur_pos - 1
                    if have_multibyte_char_at(self.command, self._private_cur_pos) then self._private_cur_pos = self._private_cur_pos - 1 end
                end
            elseif key == "d" then
            ...
            elseif key == "p" then
                if self.history_path and history_index > 1 then
                    history_index = history_index - 1
                    self.command = data.history[self.history_path].table[history_index]
                    self._private_cur_pos = #self.command + 2
                end
            elseif key == "n" then
                if self.history_path and history_index < history_items(self.history_path) then
                    history_index = history_index + 1
                    self.command = data.history[self.history_path].table[history_index]
                    self._private_cur_pos = #self.command + 2
                elseif self.history_path and history_index == history_items(self.history_path) then
                    history_index = history_index + 1
                    self.command = ""
                    self._private_cur_pos = 1
                end
            elseif key == "e" then self._private_cur_pos = #self.command + 1
            elseif key == "r" then
                if self.history_path and data.history[self.history_path] then
                    ...
                    for i,v in (function(a,i) return itera(-1,a,i) end), data.history[self.history_path].table, history_index do
                        if v:find(self._private.search_term,1,true) ~= nil then self.command=v; history_index=i; self._private_cur_pos=#self.command+1; break end
                    end
                end
            elseif key == "s" then
                if self.history_path and data.history[self.history_path] then
            ...
            elseif key == "f" then
                if self._private_cur_pos <= #self.command then
                    if have_multibyte_char_at(self.command, self._private_cur_pos) then self._private_cur_pos = self._private_cur_pos + 2
                    else self._private_cur_pos = self._private_cur_pos + 1 end
                end
            elseif key == "h" then
                if self._private_cur_pos > 1 then
                    local offset = 0
                    if have_multibyte_char_at(self.command, self._private_cur_pos - 1) then offset = 1 end
            ...
            elseif key == "k" then self.command = self.command:sub(1, self._private_cur_pos - 1)
            elseif key == "u" then self.command = self.command:sub(self._private_cur_pos, #self.command); self._private_cur_pos = 1
            elseif key == "Prior" then
                if self.history_path and data.history[self.history_path] then
                    self._private.search_term = self.command:sub(1, self._private_cur_pos - 1) or ""
                    for i,v in (function(a,i) return itera(-1,a,i) end), data.history[self.history_path].table, history_index do
                        if v:find(self._private.search_term,1,true) == 1 then self.command=v; history_index=i; break end
                    end
                end
            ...
            elseif key == "w" or key == "BackSpace" then
                local wstart = 1; local wend = 1; local cword_start_pos = 1; local cword_end_pos = 1
                while wend < self._private_cur_pos do
                    wend = self.command:find("[{[(,.:;_-+=@/ ]", wstart)
                    if not wend then wend = #self.command + 1 end
                    if self._private_cur_pos >= wstart and self._private_cur_pos <= wend + 1 then cword_start_pos = wstart; cword_end_pos = self._private_cur_pos - 1; break end
                    wstart = wend + 1
                end
            ...
            elseif key == "Delete" then
                if self.history_path and data.history[self.history_path] and self.command == data.history[self.history_path].table[history_index] then
                    table.remove(data.history[self.history_path].table, history_index)
                    if history_index <= history_items(self.history_path) then
                        self.command = data.history[self.history_path].table[history_index]; self._private_cur_pos = #self.command + 2
                    elseif history_index > 1 then
                        history_index = history_index - 1; self.command = data.history[self.history_path].table[history_index]; self._private_cur_pos = #self.command + 2
                    else
                        self.command = ""; self._private_cur_pos = 1
                    end
                end
            end
        elseif mod.Mod1 or mod.Mod3 then
            if key == "b" then self._private_cur_pos = cword_start(self.command, self._private_cur_pos)
            elseif key == "f" then self._private_cur_pos = cword_end(self.command, self._private_cur_pos)
            elseif key == "d" then self.command = self.command:sub(1, self._private_cur_pos - 1) .. self.command:sub(cword_end(self.command, self._private_cur_pos))
                ...
                if key == "Tab" or key == "ISO_Left_Tab" then
                    if key == "ISO_Left_Tab" or mod.Shift then
                        ...
                        if ncomp == 2 then
                            ...
                            self.textbox:set_font(self.font)
                            self.textbox:set_markup(prompt_text_with_cursor{ text = command_before_comp, text_color = self.fg_cursor, cursor_color = self.bg_cursor, cursor_pos = self._private_cur_pos, cursor_ul = self.ul_cursor, select_all = self.select_all, prompt = self.prompt })
                    ...
                    ncomp = ncomp + 1
                    key = ""
                    if matches and #matches == 1 and self.autoexec then exec(self, self.exe_callback); return end
            ...
            elseif key == "Home" then self._private_cur_pos = 1
            elseif key == "End" then self._private_cur_pos = #self.command + 1
            elseif key == "BackSpace" then
                if self._private_cur_pos > 1 then
                    local offset = 0
                    if have_multibyte_char_at(self.command, self._private_cur_pos - 1) then offset = 1 end
            ...
            elseif key == "Delete" then self.command = self.command:sub(1, self._private_cur_pos - 1) .. self.command:sub(self._private_cur_pos + 1)
            elseif key == "Left" then self._private_cur_pos = self._private_cur_pos - 1
            elseif key == "Right" then self._private_cur_pos = self._private_cur_pos + 1
            elseif key == "Prior" then
                if self.history_path and history_index > 1 then
                    history_index = history_index - 1; self.command = data.history[self.history_path].table[history_index]; self._private_cur_pos = #self.command + 2
                end
            elseif key == "Next" then
                if self.history_path and history_index < history_items(self.history_path) then
                    history_index = history_index + 1; self.command = data.history[self.history_path].table[history_index]; self._private_cur_pos = #self.command + 2
                elseif self.history_path and history_index == history_items(self.history_path) then
                    history_index = history_index + 1; self.command = ""; self._private_cur_pos = 1
                end
            else
                if key:wlen() == 1 then
            ...
            if self._private_cur_pos < 1 then self._private_cur_pos = 1
            elseif self._private_cur_pos > #self.command + 1 then self._private_cur_pos = #self.command + 1 end
            self.select_all = nil
        end
        update(self)
        if self.changed_callback then self.changed_callback(self.command) end
...
function prompt.mt:__call(...) return new(...) end
